"""
This module provides functionality to access the Shazam API (https://rapidapi.com/apidojo/api/shazam)
to identify a segment of song data.

Requests are performed using the ``requests`` library, so functions sending requests to the
Shazam API may raise all exceptions a failed ``requests.post`` call can raise.
"""
...
"""amount of time to skip to the next segment if a segment isn't recognized, in seconds."""
...
def lookup(song_data: np.ndarray, apikey: str, from_start: bool = True):
    ...
    The step size is defined by the LOOKUP_OFFSET_INCREMENT constant and set to 10 seconds.
    ...
    Load a song and load metadata for it using ``lookup``::
        ...
        song, sr = librosa.load("my_cool_file.mp3", sr=44100)
...
def _format_song_data(song_data):
    """Format the song data as a mono 16-bit little endian integer array,
...
def _get_song_data_segment(song_data: np.ndarray, offset: int):
    ...
    which is set to 4 seconds as the Shazam API expects uploaded files to be 3-5 seconds long.
...
def _create_payload_from_song_data(song_data: np.ndarray):
    ...
    :param song_data: The song data, formatted as a mono 16-bit little endian integer array.
...
def _extract_value_from_metadata(track, key: str):
    ...
    Note: If an error occurs that leads to the "Metadata" section containing two sets of data with
    the same label, the first set of data is returned. This should not be able to happen.
