...
The currently analyzed song is stored in the ``current_song_*`` attributes, formatted as an offset and a duration to indicate its position in the analyzed file and the metadata options for the song, as gathered using the API calls. Once a new segment's metadata doesn't match the currently analyzed song, the current song is stored in the ``last_song_*`` attributes and can be retrieved using [``get_last_song``](#get_last_song).

The workflow of the API service, as implemented in [``identify_all_from_generator``](#identify_all_from_generator), should look as follows (pseudocode):
...
while(has_next_segment):
  result = service.get_song_options(next_segment)
  if (result is SONG_FINISHED or SONG_NOT_RECOGNISED and first_segment = False) or result is SONG_MISMATCH:
    result_segments[] = service.get_last_song()
  if (result is not SONG_EXTENDED)
    first_segment = False
result_segments[] = service.get_final_song()
```
